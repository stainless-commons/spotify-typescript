...
export class Playlists extends APIResource {
   ...
   * @example
   * ```ts
   * const playlist = await client.me.playlists.create({
   *   name: 'New Playlist',
   * });
   * ```
   */
  create(body: PlaylistCreateParams, options?: RequestOptions): APIPromise<PlaylistCreateResponse> {
    return this._client.post('/me/playlists', { body, ...options });
  }
