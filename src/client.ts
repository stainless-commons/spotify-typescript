...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['SPOTIFY_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SPOTIFY_LOG'] or 'warn' if it isn't set.
   */
...
export class Spotify {
  ...
  constructor({
    ...
    this.timeout = options.timeout ?? Spotify.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SPOTIFY_LOG'), "process.env['SPOTIFY_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('SPOTIFY_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static SpotifyError = Errors.SpotifyError;
...
Spotify.Shows = Shows;
Spotify.Episodes = Episodes;
Spotify.Audiobooks = Audiobooks;
Spotify.Me = Me;
Spotify.Chapters = Chapters;
Spotify.Tracks = Tracks;
Spotify.Search = Search;
Spotify.Playlists = Playlists;
Spotify.Users = Users;
Spotify.Browse = Browse;
Spotify.AudioFeatures = AudioFeatures;
Spotify.AudioAnalysis = AudioAnalysis;
